...
#
#  INSTALACIÓN:
#    pip install tensorflow[cuda] matplotlib numpy
#
# =============================================================
...
def correr_experimento(porcentaje_train, x_flat, x_cnn, y_todo):
    ...
    ax[0].set(title="Tamaño vs Accuracy", xlabel="Tamaño del modelo",
    ...
    ax[1].set(title="# Parámetros", xlabel="Tamaño", ylabel="Parámetros")
    ...
    ax2.axhline(0.95, color="red", ls="--", label="Meta 95%")
    ax2.set(title=f"MLP — Curvas de Entrenamiento ({titulo})",
            xlabel="Época", ylabel="Accuracy")
    ax2.legend(); ax2.grid(True, alpha=0.3)
    plt.tight_layout(); guardar(fig2, "mlp_curvas_entrenamiento.png")

    # Gráfica CNN: tamaño vs accuracy
    ...
    ax3[0].axhline(95, color="red", ls="--", label="Meta 95%")
    for e, a in zip(etiq, acc_cnn):
        ax3[0].annotate(f"{a:.2f}%", (e, a), textcoords="offset points",
                        xytext=(0,10), ha="center", fontsize=11, fontweight="bold")
    ax3[0].set(title="Tamaño vs Accuracy", xlabel="Tamaño del modelo",
               ylabel="Accuracy (%)", ylim=(75, 102))
    ...
    ax3[1].set(title="# Parámetros", xlabel="Tamaño", ylabel="Parámetros")
    ...
    ax4.axhline(0.95, color="red", ls="--", label="Meta 95%")
    ax4.set(title=f"CNN — Curvas de Entrenamiento ({titulo})",
            xlabel="Época", ylabel="Accuracy")
    ...
    ax5.axhline(95, color="red", ls="--", lw=1.5, label="Meta 95%")
    ax5.set(title=f"Comparación MLP vs CNN — {titulo}",
            ylabel="Accuracy (%)", ylim=(75, 102))
...
ax6.axhline(y=95, color="red", linestyle="--", linewidth=1.5, label="Meta 95%")
ax6.set_title("Tamaño del Training vs Accuracy  (MLP y CNN)", fontsize=13, fontweight="bold")
ax6.set_xlabel("Porcentaje de datos usados para entrenar", fontsize=11)
ax6.set_ylabel("Accuracy en prueba (%)", fontsize=11)
ax6.set_ylim(75, 102)
...
fig8.suptitle("Tamaño del Modelo vs Tiempo de Inferencia",
              fontsize=13, fontweight="bold")

# Panel izquierdo: línea con puntos (parámetros vs tiempo)
...
# Líneas separadas MLP y CNN para que se vea la tendencia
...
ejes8[0].set_title("# Parámetros vs Tiempo de inferencia")
ejes8[0].set_xlabel("Número de parámetros")
ejes8[0].set_ylabel("Tiempo por inferencia (ms)")
ejes8[0].legend(fontsize=8, loc="upper left")
ejes8[0].grid(True, alpha=0.3)

# Panel derecho: barras horizontales comparando los 6 modelos
...
ejes8[1].set_title("Comparación directa — Tiempo por modelo")
ejes8[1].set_xlabel("Tiempo por inferencia (ms)")
ejes8[1].grid(True, alpha=0.3, axis="x")
ejes8[1].invert_yaxis()   # MLP 100% arriba
